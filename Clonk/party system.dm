/*
If you are reading through this, you should probably have a decent understanding of DM,
as most of the basic things won't be noted. This is UNTESTED without a graphical display, so I cannot guarantee
it will work if you have it disabled! for best results, use an interface arrangement similar to the one I provided.
*/
...
var
	...
	GRAPHICAL_DISPLAY=0		//if you choose to use the graphical display, or rearrange it, whatever the case may be
...
mob/var
	...
	level      - this is the players current level, not necessary if you made an alternative to levels
	...
	maxexp     - this variable is used to check for level ups, against the current exp value
	*/
	partydatum
	//gold
	level
	...
	level_up() is just there for an example, although, for expgive, if you want to use party sharing correctly,
...
mob/proc
	level_up()
		...
		//	src << "You gained a level!"
			//lvl++
		levelCheck()
		spawn()if(partydatum&&partydatum!="pending"){var/party/p=partydatum;p.updatestats()}
	expgain(expz as num)
...
party
	...
	var/tmp
		...
		levelres   = 150	//we could restrict our party so people 15 levels higher than us can't get in
		levelreq   = 0		//we can restrict our party so people 15 levels lower than us can't get in
	...
	proc
		...
		*/
		...
		requestjoin(mob/wantstojoin)
			...
			if(wantstojoin.level<levelreq)       {wantstojoin<<"<b><font color = green>You are under that parties level requirement.</font></b>";wantstojoin.partydatum=null;return}
			if(wantstojoin.level>levelres)       {wantstojoin<<"<b><font color = green>You are over that parties level restriction.</font></b>";wantstojoin.partydatum=null;return}
		...
		updatestats()
			...
			for(var/n=0 to members.len-1)
				var/mob/m=members[++n]
				members << output("Level [m.level]","Partywindow.peoplegrid:[++num],[col]")
				col++
				if(num>1)num=0
...
mob
	...
	verb
		...
		A simple rundown:
		optionsparty
			*allows you to set the party options, gold sharing, exp sharing, tile range to share, level requirement,
			 level restriction, or member limit
			*leader only
		inviteparty
		...
		*/
		optionsparty()
			...
			alertopen=1
			switch(input(src,"What options would you like to change?","Party options")in list("Gold sharing","Exp sharing","Sharing range","Level requirement","Level restriction","Member limit","Cancel"))
				if("Gold sharing")
		...
		inviteparty()
			...
			if(x.level<p.levelreq){src<<"<b><font color = green>[x] is under your party's level requirement.</font></b>";alertopen=0;return}
			if(x.level>p.levelres){src<<"<b><font color = green>[x] is over your party's level restriction.</font></b>";alertopen=0;return}
		...
		makeparty()
			...
			if(lentext(x)<3){src << "<b><i>Party name too short. Please try again with a longer name.";alertopen=0;return}
			p.createparty(x,src)
			alertopen=0
		...
		joinparty()
			...
			if(partydatum)   {src << "<b><i>You are already in a party!";return}
			alertopen=1
			var/thename=input(src,"Which party would you like to join?","Joining party")as text
